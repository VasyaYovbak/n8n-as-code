...
export class N8nApiClient {
    ...
    private normalizeExecutionSummary(execution: any): IExecutionSummary {
        return {
            ...execution,
            id: String(execution.id ?? execution.executionId ?? 'unknown'),
            finished: execution.finished != null ? Boolean(execution.finished) : undefined,
            mode: execution.mode != null ? String(execution.mode) : undefined,
            ...
            startedAt: execution.startedAt != null ? String(execution.startedAt) : undefined,
            stoppedAt: execution.stoppedAt != null ? String(execution.stoppedAt) : null,
            workflowId: execution.workflowId != null ? String(execution.workflowId) : undefined,
            workflowName: execution.workflowName != null ? String(execution.workflowName) : undefined,
    ...
    async listExecutions(options: {
        ...
        return {
            data: data.map((execution: any): IExecutionSummary => this.normalizeExecutionSummary(execution)),
            nextCursor: page.nextCursor ?? null,
            total: typeof page.count === 'number'
                ? page.count
    ...
    async getExecution(id: string, options: boolean | { includeData?: boolean } = {}): Promise<IExecutionDetails> {
        const includeData = typeof options === 'boolean' ? options : options.includeData;
        ...
        return {
            ...this.normalizeExecutionSummary(execution),
            data: execution.data,
